...
<body>
    ...
    <div class="signup-card">
        ...
        <!-- Formulir Eksekusi -->
        <form action="{{ route('signup.perform') }}" method="POST" id="signupForm">
            @csrf

            <!-- 1. Kolom Input Nama Lengkap Asli -->
            <div class="input-group">
                <label for="fullname">Nama Lengkap</label>
                <input type="text" id="fullname" name="fullname" value="{{ old('fullname') }}" placeholder="Masukkan nama lengkap Anda" required>
                @error('fullname')
            ...
            <div class="input-group">
                ...
                <input type="email" id="email" name="email" value="{{ old('email') }}" placeholder="contoh@peternak.com" required>
                @error('email')
                    <span style="color: #ff7e5f; font-size: 12px; margin-top: 4px; display: block;">{{ $message }}</span>
                @enderror
            </div>
        ...
        <div class="extra-links">
            ...
            <p style="margin-top: 10px;">
                <a href="{{ route('index') }}" style="font-size: 13px; opacity: 0.8;" title="Kembali ke Layar Beranda">← Kembali ke
                    Halaman Utama</a>
            </p>
        </div>
    </div>
