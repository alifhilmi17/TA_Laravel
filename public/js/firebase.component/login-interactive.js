...
function initializeLogin() {
        ...
        btnRolePetugas.addEventListener("click", () => {
            ...
            if (formSubtitle)
                formSubtitle.innerText =
                    "Mohon isi data akses Petugas Anda untuk memasuki portal.";
    ...
    if (btnRoleAdmin && card) {
        btnRoleAdmin.addEventListener("click", () => {
            ...
            if (formSubtitle)
                formSubtitle.innerText =
                    "Mohon isi data akses Admin / Owner Anda untuk memasuki portal.";
    ...
    if (loginForm) {
        loginForm.addEventListener("submit", async (e) => {
            ...
            try {
                console.log(
                    "Memulai proses masuk untuk username:",
                    usernameVal,
                    "dengan role pilihan:",
                    selectedRole,
                );
                ...
                if (selectedRole === "admin") {
                    ...
                    if (!snapAdmin.empty) {
                        ...
                        console.log(
                            "Ditemukan di koleksi 'admin' dengan data:",
                            userData,
                        );
                    } else {
                        // Jika tidak ada di koleksi admin, cari di koleksi user (fallback jika data admin disinkronisasi)
                        console.log(
                            "Tidak ditemukan di koleksi 'admin', mencari di koleksi 'user'...",
                        );
                        ...
                        if (!snapUser.empty) {
                            ...
                            console.log(
                                "Ditemukan di koleksi 'user' untuk pencarian admin:",
                                userData,
                            );
                        }
                    }
                    ...
                    if (!snapUser.empty) {
                        ...
                        userRole = userData.role || "user";
                        console.log(
                            "Ditemukan di koleksi 'user' dengan data:",
                            userData,
                        );
                    }
                }
                ...
                console.log(
                    "Mencocokkan email & kata sandi via Firebase Auth...",
                );
                ...
                const chickenOverlay =
                    document.getElementById("chicken-overlay");
                if (chickenOverlay) {
                ...
                setTimeout(() => {
                    if (finalRedirectRole === "admin") {
                        ...
                        window.location.href =
                            "admin.frontend/admin-core/admin.html";
                    } else {
                ...
                if (
                    ...
                    errorMsg =
                        "Username atau sandi yang Anda masukkan salah, mohon periksa kembali.";
                } else if (error.code === "auth/role-mismatch-petugas") {
                    errorMsg =
                        "Akun Anda terdaftar sebagai Petugas. Silakan masuk kembali menggunakan portal Petugas.";
                } else if (error.code === "auth/role-mismatch-admin") {
                    errorMsg =
                        "Akun Anda terdaftar sebagai Admin/Owner. Silakan masuk kembali menggunakan portal Admin.";
                } else if (error.code === "auth/too-many-requests") {
                    errorMsg =
                        "Terlalu banyak percobaan gagal beruntun. Coba lagi beberapa saat.";
                } else if (error.code === "permission-denied") {
                    errorMsg =
                        "Database tertutup! Buka Firebase Console > Firestore > Rules, ubah bagian 'allow read' menjadi 'if true;' agar sistem bisa mencari Username.";
                } else {
