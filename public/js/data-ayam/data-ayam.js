...
document.addEventListener("DOMContentLoaded", () => {
    // --- KODE FIREBASE ---
    unsubscribeAyam = onSnapshot(
        query(ayamCollection, orderBy("tglMasuk", "desc")),
        (snapshot) => {
    ...
    unsubscribeKesehatan = onSnapshot(
        kesehatanCollection,
        (snapshot) => {
...
function updateQuickStats(filteredData = null) {
    ...
    dataToCalculate.forEach((ayam) => {
        ...
        if (status === "aktif") {
            ...
            const sakitDiBatchIni = dataKesehatan
                .filter(
                    (x) =>
                        x.batchId === ayam.id && x.status === "Dalam Perawatan",
                )
    ...
    if (elKandangTerisi)
        elKandangTerisi.innerText = setKandang.size + " Kandang";
}
...
window.openAyamModal = function () {
    ...
    newAwal.addEventListener("input", () => {
        if (document.getElementById("ayamId").value === "") {
            // Hanya saat tambah baru
...
window.saveAyamData = async function (event) {
    ...
    const jumlahAwal =
        parseInt(document.getElementById("jumlahAwal").value) || 0;
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            try {
                ...
                const kesSnapshot = await getDocs(
                    collection(db, "kesehatan_ayam"),
                );
                const kesPromises = [];
                kesSnapshot.forEach((docSnap) => {
                    if (docSnap.data().batchId === id)
                        kesPromises.push(deleteDoc(docSnap.ref));
                });
                ...
                prodSnapshot.forEach((docSnap) => {
                    if (docSnap.data().batchId === id)
                        prodPromises.push(deleteDoc(docSnap.ref));
                });
                ...
                */
                ...
                Swal.fire(
                    "Terhapus!",
                    "Data batch beserta riwayatnya telah dihapus.",
                    "success",
                );
            } catch (error) {
                Swal.fire(
...
window.searchTable = function () {
    ...
    updateQuickStats(
        filteredResults.length > 0 || input ? filteredResults : undefined,
    );
};
...
window.downloadLaporanCSV = function () {
    ...
    // Header Tabel CSV
    let csvContent =
        "ID Batch,Tanggal Masuk,Jenis Telur Ayam,Populasi Awal,Sisa Ayam,Kandang,Status\n";
