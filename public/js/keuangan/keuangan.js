...
async function loadProductionRelatedData() {
    try {
        ...
        dataAyam = ayamSnapshot.docs.map((doc) => ({
            id: doc.id,
            ...doc.data(),
        }));
        ...
        dataProduksi = prodSnapshot.docs.map((doc) => ({
            id: doc.id,
            ...doc.data(),
        }));
...
function precomputeProductionWeeks() {
    ...
    Object.keys(batchGroups).forEach((batchId) => {
        ...
        batchGroups[batchId].forEach((prod, index) => {
            ...
            if (prod.tanggal < wGroup.tanggalAwal)
                wGroup.tanggalAwal = prod.tanggal;
            if (prod.tanggal > wGroup.tanggalAkhir)
                wGroup.tanggalAkhir = prod.tanggal;
        });
    });
}
...
window.loadBatchWeeks = function () {
    ...
    if (!weeks || Object.keys(weeks).length === 0) {
        weekSelect.innerHTML =
            '<option value="" disabled selected>— tidak ada data produksi —</option>';
        weekSelect.disabled = true;
        document.getElementById("weeklyProductionSummary").style.display =
            "none";
        return;
    }
...
window.showWeekProductionSummary = function () {
    ...
    const dateAwalIndo = window.formatTanggal(wGroup.tanggalAwal);
    const dateAkhirIndo = window.formatTanggal(wGroup.tanggalAkhir);
    document.getElementById("summaryPeriode").innerText =
        `${dateAwalIndo} s/d ${dateAkhirIndo}`;
    document.getElementById("summaryBaik").innerText =
        `${wGroup.telurBaik.toLocaleString("id-ID")} Butir`;
    document.getElementById("summaryCacat").innerText =
        `${wGroup.telurCacat.toLocaleString("id-ID")} Butir`;
    document.getElementById("summaryTotal").innerText =
        `${wGroup.totalTelur.toLocaleString("id-ID")} Butir`;
...
window.switchTrxSource = function (source) {
    ...
    if (source === "manual") {
        ...
        if (lblTrxBatch)
            lblTrxBatch.innerHTML =
                'Target Batch Ayam <small style="font-weight: normal; color: #64748b;">(Opsional)</small>';
        if (trxBatch) {
        ...
        if (lblTrxBatch)
            lblTrxBatch.innerHTML =
                'Batch Ayam <span style="color: #ef4444;">*</span>';
        if (trxBatch) {
...
async function handleFormSubmit(event) {
    ...
    if (trxSource === "produksi") {
        ...
        payload.hargaPerPapan =
            parseFloat(
                priceInput.value.replace(/\./g, "").replace(/,/g, "."),
            ) || 0;
    } else {
...
function renderTable() {
    ...
    if (viewMode === "tipe") {
        ...
        const pengeluaranList = filtered.filter(
            (t) => t.tipe === "pengeluaran",
        );

        // 1. Kelompok Pemasukan
        const subtotalPemasukan = pemasukanList.reduce(
            (sum, t) => sum + t.jumlah,
            0,
        );
        ...
        const subtotalPengeluaran = pengeluaranList.reduce(
            (sum, t) => sum + t.jumlah,
            0,
        );
