    ...
    onSnapshot(q, (snapshot) => {
        ...
        if (isInitialLoad) {
            ...
            if (pending.length > 0) {
                ...
                const title =
                    tinggi.length > 0
                        ? `${tinggi.length} Pengingat Mendesak!`
                        : `${pending.length} Pengingat Pending`;
...
function renderLiveStockWidget(stockMap) {
    ...
    keys.forEach((jenis) => {
        ...
        const cardClass = isLow
            ? "stock-item-card low-stock animate__animated animate__pulse animate__infinite"
            : "stock-item-card";
...
function renderReminders() {
    ...
    const countPending = dataReminders.filter(
        (r) => r.status === "Pending" || r.status === "Usulan" || !r.status,
    ).length;
    const countSelesai = dataReminders.filter(
        (r) => r.status === "Selesai",
    ).length;
    ...
    if (activeTab === "pending") {
        filteredData = dataReminders.filter(
            (r) => r.status === "Pending" || r.status === "Usulan" || !r.status,
        );
    } else {
    ...
    filteredData.forEach((r) => {
        ...
        const beaconHtml = showBeacon
            ? '<span class="pulse-beacon" title="Mendesak!"></span>'
            : "";
        ...
        if (isSelesai) {
            badgeStatus =
                '<span class="badge badge-selesai">✅ Selesai Di-restock</span>';
        } else if (isUsulan) {
            badgeStatus =
                '<span class="badge badge-usulan">💡 Usulan Baru</span>';
        } else {
        ...
        const dateStr = r.tglBatas
            ? new Date(r.tglBatas).toLocaleDateString("id-ID", {
                  day: "numeric",
                  month: "long",
                  year: "numeric",
              })
...
window.openReminderModal = function () {
    ...
    document.getElementById("tglReminder").value =
        window.getLocalDateString(tomorrow);

    document.getElementById("modalTitle").innerText =
        currentUserRole === "admin"
            ? "Buat Reminder Baru"
            : "Ajukan Usulan Kebutuhan Pakan";
    ...
    Swal.fire({
        ...
        if (res.isConfirmed) {
            try {
                ...
                Swal.fire(
                    "Selesai",
                    "Pakan telah ditandai berhasil direstock.",
                    "success",
                );
            } catch (err) {
                Swal.fire("Error", err.message, "error");
    ...
    Swal.fire({
        ...
        if (res.isConfirmed) {
            try {
                ...
                Swal.fire(
                    "Terhapus",
                    "Reminder telah berhasil dihapus.",
                    "success",
                );
            } catch (err) {
                Swal.fire("Error", err.message, "error");
