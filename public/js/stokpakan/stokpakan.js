    ...
    onAuthStateChanged(auth, async (user) => {
        ...
        try {
            ...
            if (adminSnap.exists()) {
                ...
                currentUserName =
                    adminSnap.data().fullname ||
                    adminSnap.data().username ||
                    "Admin";
            } else {
                // Cek koleksi 'user'
                ...
                if (userSnap.exists()) {
                    ...
                    currentUserName =
                        userData.fullname || user.displayName || "Petugas";
                    const role = (userData.role || "petugas")
                        .trim()
                        .toLowerCase();
                    currentUserRole =
                        role === "admin" ||
...
window.calculateTotalHargaPakan = function () {
    ...
    const price =
        parseFloat(document.getElementById("hargaPakanKg").value) || 0;
...
window.toggleJenisPakanInput = function () {
    ...
    if (tipe === "Keluar") {
        ...
        const uniqueFeeds = [
            ...new Set(
                dataPakan.filter((p) => p.tipe === "Masuk").map((p) => p.jenis),
            ),
        ];
...
window.openPakanModal = function (type = "") {
    ...
    if (type) {
        ...
        document.getElementById("modalTitlePakan").innerText =
            type === "Masuk"
                ? "➕ Tambah Stok Pakan"
...
window.savePakanData = async function (event) {
    ...
    const tipe = document.getElementById("tipePakan").value;
    const jumlah =
        parseFloat(document.getElementById("jumlahPakan").value) || 0;
    ...
    if (tipe === "Keluar") {
        ...
        if (id !== "") {
            ...
            if (
                itemLama &&
                itemLama.tipe === "Keluar" &&
                itemLama.jenis === jenisDipilih
            ) {
    ...
    const batchLabelVal =
        batchIdVal && batchPakanEl
            ? batchPakanEl.options[batchPakanEl.selectedIndex].text
            : "";
    ...
    const payload = {
        ...
        jenis:
            tipe === "Keluar"
                ? document.getElementById("jenisPakanSelect").value
    ...
    try {
        if (id === "") {
            ...
            if (tipe === "Masuk" && catatKeuangan && catatKeuangan.checked) {
                const hargaPerKg =
                    parseFloat(document.getElementById("hargaPakanKg").value) ||
                    0;
...
window.editPakan = function (id) {
    ...
    document.getElementById("modalTitlePakan").innerText =
        "✏️ Edit Data " + (item.tipe === "Masuk" ? "Pakan Masuk" : "Pemakaian");
    document.getElementById("pakanModal").classList.add("show");
};
...
window.deletePakan = function (id) {
    ...
    if (!isAdmin && item && item.dicatatOleh !== currentUserName) {
        Swal.fire(
            "Akses Ditolak",
            "Anda hanya dapat menghapus data yang Anda catat sendiri.",
            "error",
        );
        return;
    }
        ...
        if (result.isConfirmed) {
            ...
            if (jenisPakan) {
                try {
                    ...
                    const allRemindersSnapshot = await getDocs(
                        collection(db, "restock_reminders"),
                    );
                    ...
                    allRemindersSnapshot.forEach((docSnap) => {
                        ...
                        if (
                            data.jenisPakan &&
                            data.jenisPakan.trim().toLowerCase() ===
                                cleanPakanJenis
                        ) {
            ...
            Swal.fire(
                "Terhapus!",
                "Data pakan dan pengingat terkait berhasil dihapus.",
                "success",
            );
        }
    });
...
function renderTable() {
    ...
    const filtered = dataPakan.filter(
        (p) =>
            p.tipe === "Masuk" &&
            (!filterBulan || p.tanggal.startsWith(filterBulan)),
    );
...
function updateQuickStats() {
    ...
    if (document.getElementById("totalPakanMasuk"))
        document.getElementById("totalPakanMasuk").innerText =
            masuk.toLocaleString("id-ID") + " Kg";
    if (document.getElementById("detailPakanTersedia"))
        document.getElementById("detailPakanTersedia").innerText =
            pakanTersediaText;
    if (document.getElementById("totalPakanKeluar"))
        document.getElementById("totalPakanKeluar").innerText =
            keluar.toLocaleString("id-ID") + " Kg";
    ...
    if (document.getElementById("pemakaianBulanIni"))
        document.getElementById("pemakaianBulanIni").innerText =
            pemakaianBulanIniCount.toLocaleString("id-ID") + " Kg";
}
