...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    await Promise.all([loadPredictionHistory(), loadSettings()]);
...
function loadBatchAyam() {
    ...
    onSnapshot(
        ...
        (snapshot) => {
            ...
            const batchAktif = batchDataAyam.filter(
                (b) => b.status === "Aktif",
            );
            ...
            if (batchAktif.length === 0) {
                selectEl.innerHTML =
                    '<option value="" disabled selected>⚠️ Tidak ada batch aktif</option>';
                hiddenPopulasi.value = 0;
                if (infoEl)
                    infoEl.textContent =
                        "Belum ada batch ayam aktif. Silakan tambah data di halaman Data Ayam.";
                return;
            }
            ...
            let totalSemua = 0;
            batchAktif.forEach(
                (b) => (totalSemua += parseInt(b.sisaAyam) || 0),
            );
            ...
            batchAktif.forEach((b) => {
                ...
                const jenisAyam = b.jenis
                    ? ` - ${b.jenis.charAt(0).toUpperCase() + b.jenis.slice(1)}`
                    : "";
...
function updateMingguBasisDropdown() {
    ...
    if (!selectedBatchId) {
        mingguSelect.innerHTML =
            '<option value="LATEST" selected>📊 Data Terkini (7 Hari Terakhir)</option>';
        return;
    }
    ...
    if (selectedBatchId === "ALL") {
        ...
        filteredData = dataProduksi.filter(
            (p) => p.batchId === selectedBatchId,
        );
    }
    ...
    mingguSelect.innerHTML =
        '<option value="LATEST">📊 Data Terkini (7 Hari Terakhir)</option>';
...
function initRealtimeConfigListeners() {
    ...
    if (inputPakan)
        inputPakan.addEventListener("input", updateRealtimeConfigCalculations);
    if (inputHargaPakan)
        inputHargaPakan.addEventListener(
            "input",
            updateRealtimeConfigCalculations,
        );
    if (inputHargaTelur) {
...
function updateRealtimeConfigCalculations() {
    ...
    const hargaPakan =
        parseInt(hargaPakanStr.replace(/\./g, "").replace(",", ".")) || 0;
    ...
    if (populasi > 0 && pakanPerEkor > 0) {
        ...
        if (rtPakanKg)
            rtPakanKg.textContent = `${totalPakanKg.toLocaleString("id-ID")} Kg / Hari`;
        if (rtBiayaPakan)
            rtBiayaPakan.textContent = `Rp ${Math.round(biayaPakan).toLocaleString("id-ID")} / Hari`;
    ...
    if (selectBatch && selectBatch.value && selectBatch.value !== "") {
        if (selectBatch.value === "ALL") {
            if (rtBatchAyam)
                rtBatchAyam.textContent = `${populasi.toLocaleString("id-ID")} Ekor`;
            ...
            const batchInfo = batchDataAyam.find(
                (b) => b.id === selectBatch.value,
            );
            if (batchInfo) {
                if (rtBatchAyam)
                    rtBatchAyam.textContent = `${(parseInt(batchInfo.sisaAyam) || 0).toLocaleString("id-ID")} Ekor`;
                if (rtBatchKandang)
                    rtBatchKandang.textContent = batchInfo.kandang || "-";
                if (rtBatchTgl) {
                    const formattedDate = batchInfo.tglMasuk
                        ? new Date(batchInfo.tglMasuk).toLocaleDateString(
                              "id-ID",
                              {
                                  day: "numeric",
                                  month: "short",
...
function loadProduksiData() {
    ...
    onSnapshot(
        q,
        (snapshot) => {
            dataProduksi = snapshot.docs.map((d) => ({
                id: d.id,
                ...d.data(),
            }));
...
function loadKesehatanData() {
    onSnapshot(
        kesehatanCollection,
        (snapshot) => {
            dataKesehatan = snapshot.docs.map((d) => ({
                id: d.id,
                ...d.data(),
            }));
        },
...
function loadKeuanganData() {
    onSnapshot(
        keuanganCollection,
        (snapshot) => {
            dataKeuangan = snapshot.docs.map((d) => ({
                id: d.id,
                ...d.data(),
            }));
        },
...
window.autoFillFromBatch = function () {
    ...
    if (dataToFill.length < periodeMA) {
        ...
        const basisLabel =
            selectedMingguVal === "LATEST"
                ? "Data Terkini (7 Hari Terakhir)"
                : `Minggu ke-${selectedMingguVal}`;
        Swal.fire({
            icon: "success",
...
function renderHistoricalInputs() {
    ...
    for (let i = totalHistoryDays - 1; i >= 0; i--) {
        ...
        let labelClassProfit = isToday
            ? "hist-input-today-label-keuntungan"
            : "";
...
function animateValue(
    ...
    const step = (timestamp) => {
        ...
        if (isCurrency) {
            valueStr =
                prefix + Math.floor(current).toLocaleString("id-ID") + suffix;
        } else {
            if (end % 1 !== 0) {
...
window.calculatePrediction = function (event) {
    ...
    const pakanPerEkor = parseFloat(
        document.getElementById("pakanPerEkor").value,
    ); // dalam satuan Gram (g)
    ...
    const hargaPakan =
        parseInt(hargaPakanStr.replace(/\./g, "").replace(",", ".")) || 0; // Rp per kg
    const hargaTelur =
        parseInt(hargaTelurStr.replace(/\./g, "").replace(",", ".")) || 0; // Rp per papan
    ...
    for (let i = startIndex; i < totalHistoryDays; i++) {
        let cleanValStr = (inputs[i].value || "")
            .replace(/\./g, "")
            .replace(",", ".");
    ...
    for (let i = startIndex; i < totalHistoryDays; i++) {
        let cleanValStr = (inputsProfit[i].value || "")
            .replace(/\./g, "")
            .replace(",", ".");
        let val = parseFloat(cleanValStr);
        if (isNaN(val)) {
    ...
    let startActual = 0;
    while (
        startActual < totalHistoryDays &&
        isNaN(parseFloat(inputs[startActual].value))
    ) {
        startActual++;
    }
    ...
    for (let i = startActual; i < totalHistoryDays; i++) {
        let cleanProdStr = (inputs[i].value || "")
            .replace(/\./g, "")
            .replace(",", ".");
        let cleanProfStr = (inputsProfit[i].value || "")
            .replace(/\./g, "")
            .replace(",", ".");
    ...
    let fullHistoryKg = fullHistoryButir.map(
        (butir) => butir / konversiButirPerKg,
    );
    ...
    let avgProfitOffset =
        validOffsetDays > 0 ? totalOffset / validOffsetDays : 0;
    ...
    let maResult = window.calculateMovingAverage(
        fullHistoryKg,
        periodeMA,
        7,
        penaltyFactor,
    ...
    const selectedBatchIdForEval = batchSelectForEval
        ? batchSelectForEval.value
        : "";
    ...
    if (selectedBatchIdForEval) {
        ...
        if (selectedBatchIdForEval === "ALL") {
            ...
            dbFiltered = dataProduksi.filter(
                (p) => p.batchId === selectedBatchIdForEval,
            );
        }
    ...
    if (keuntungan < 0) {
        ...
        highlightCard
            .querySelector("p")
            .classList.remove("highlight-text-green");
    ...
    let totalPrediksiButir7Hari = Math.round(
        totalPrediksiKg7Hari * konversiButirPerKg,
    );
    ...
    if (selectedMingguVal === "LATEST") {
        ...
        if (selectedBatchId && selectedBatchId !== "ALL") {
            filtered = dataProduksi.filter(
                (p) => p.batchId === selectedBatchId,
            );
        }
        filtered.sort((a, b) => a.tanggal.localeCompare(b.tanggal));
        const latestIndex = filtered.length - 1;
        const currentLatestWeek =
            latestIndex >= 0 ? Math.floor(latestIndex / 7) + 1 : 1;
    ...
    const batchSelect = document.getElementById("populasiBatch");
    const selectedBatchLabel = batchSelect
        ? batchSelect.options[batchSelect.selectedIndex].textContent
        : "-";
    lastPredictionData = {
...
function generateRekomendasi(data) {
    ...
    const produksiKemarin =
        historyLen >= 2 ? fullHistoryButir[historyLen - 2] : produksiHariIni;
    const perubahanProduksi = produksiHariIni - produksiKemarin;
    const persentasePerubahan =
        produksiKemarin > 0 ? (perubahanProduksi / produksiKemarin) * 100 : 0;
    ...
    let trenText =
        trenHdp > 0
            ? `(Prediksi Naik +${trenHdp.toFixed(1)}%)`
            : trenHdp < 0
              ? `(Prediksi Turun ${Math.abs(trenHdp).toFixed(1)}%)`
              : `(Prediksi Stabil)`;
...
function updateChart(
    ...
    for (let i = historyKg.length; i > 0; i--) {
        ...
        dataHistoryKeuntungan.push(
            historyKeuntungan[historyKeuntungan.length - i],
        );
    ...
    dataPredictKeuntungan[dataPredictKeuntungan.length - 1] =
        historyKeuntungan[historyKeuntungan.length - 1];
                        ...
                        label: function (context) {
                            ...
                            if (context.parsed.y !== null) {
                                if (context.datasetIndex < 2) {
                                    let butir = Math.round(
                                        context.parsed.y * 16,
                                    );
                                    label +=
                                        context.parsed.y.toFixed(2) +
                                        " Kg (" +
                                        butir.toLocaleString("id-ID") +
                                        " Butir)";
                                } else {
...
function renderHistoryTable(histories) {
    ...
    histories.forEach((h, idx) => {
        ...
        const rekTitleShort =
            rekTitle.length > 45 ? rekTitle.substring(0, 42) + "..." : rekTitle;
    ...
    try {
                    ...
                    const actionsStr =
                        r.actions && r.actions.length > 0
                            ? r.actions
                                  .map(
                                      (a) =>
                                          `<li style="font-size:0.82rem; margin-bottom:4px; color:#4a5568;">${a}</li>`,
                                  )
                                  .join("")
                            : "";
                    return `
        ...
        if (h.proyeksi7HariKg && h.proyeksi7HariKg.length > 0) {
            ...
            for (let i = 0; i < h.proyeksi7HariKg.length; i++) {
                ...
                const factor =
                    h.prediksiBesokKg > 0
                        ? h.prediksiBesokButir / h.prediksiBesokKg
                        : 16;
        ...
        const profitStyle =
            h.keuntungan < 0 ? "color: #e74c3c;" : "color: #27ae60;";

        // Add MAE & Akurasi logic
        let akurasi =
            h.akurasiModel !== undefined && h.akurasiModel !== null
                ? h.akurasiModel
                : 0;
        if (
            akurasi === 0 &&
            h.keuntungan &&
            h.estimasiPendapatan &&
            !h.hasOwnProperty("akurasiModel")
        ) {
            const rasio = (h.keuntungan / h.estimasiPendapatan) * 100;
            akurasi = Math.max(0, Math.min(100, 50 + rasio));
        }
        let maeText =
            h.maeButir !== undefined && h.maeButir !== null
                ? `± ${Math.round(h.maeButir)} Butir`
                : "-";
        let akurasiText =
            h.akurasiModel !== null ? `${Math.round(akurasi)}%` : "N/A";
        ...
        Swal.fire(
            "Error",
            "Gagal memuat detail histori: " + error.message,
            "error",
        );
    }
};
...
window.clearPredictionHistory = async function () {
    ...
    try {
        ...
        const deletePromises = snapshot.docs.map((d) =>
            deleteDoc(doc(db, "prediksi_history", d.id)),
        );
        ...
        Swal.fire(
            "Error",
            "Gagal menghapus histori: " + error.message,
            "error",
        );
    }
};
...
window.downloadPrediksiCSV = function () {
    if (!lastPredictionData) {
        Swal.fire(
            "Belum Ada Data",
            "Lakukan analisis prediksi terlebih dahulu sebelum mengunduh.",
            "info",
        );
        return;
    }
    ...
    csv +=
        "Hari,Produksi (Kg),Produksi (Butir),Proyeksi Laba/Rugi (Rp),Status\n";
    ...
    if (d.rekomendasi && d.rekomendasi.length > 0) {
        d.rekomendasi.forEach((r, idx) => {
            const actionsStr = r.actions
                ? r.actions.map((a) => sanitize(a)).join(" | ")
                : "-";
