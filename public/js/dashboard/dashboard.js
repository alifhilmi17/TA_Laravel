...
async function loadDashboardData() {
    ...
    const [populasiSnap, produksiSnap, pakanSnap, keuanganSnap] =
        await Promise.all([
    ...
    if (telurEl)
        telurEl.innerText = `${totalTelurToday.toLocaleString("id-ID")} Butir`;

    const ayamEl = document.getElementById("stat-ayam");
    if (ayamEl)
        ayamEl.innerText = `${totalAyamAktif.toLocaleString("id-ID")} Ekor`;

    const mortEl = document.getElementById("stat-mortalitas");
    if (mortEl)
        mortEl.innerText = `${mortalitasToday.toLocaleString("id-ID")} Ekor`;
    ...
    if (outEl)
        outEl.innerText = `Rp ${pengeluaranBulan.toLocaleString("id-ID")}`;
